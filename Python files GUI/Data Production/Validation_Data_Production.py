...
def readData():
	...
	with open("AllAggregated_NoDistinction_True_Data_Validation.csv", "r") as f:
...
def readAndSortData():
    ...
    with open("AllAggregated_True_Data_Validation.csv", "r") as f:
        reader = csv.reader(f, delimiter="\t")
        print(reader)
        for row in list(reader)[1:]:
            l = ', '.join(row)
...
def runMultipleReplications(ListOfReplications):
	...
	writeCSVfileAllData(synthetic_Data_HDV,"Validation_Synthetic_Data_HDV")
	writeCSVfileAllData(synthetic_Data_CAV,"Validation_Synthetic_Data_CAV")
	writeCSVfileAllData(synthetic_Data_No_Distinction,"Validation_Synthetic_Data_No_Distinction")

	writeCSVfileAllData(synthetic_Data_HDV_err,"Validation_Synthetic_Data_HDV_err")
	writeCSVfileAllData(synthetic_Data_CAV_err,"Validation_Synthetic_Data_CAV_err")
	writeCSVfileAllData(synthetic_Data_No_Distinction_err,"Validation_Synthetic_Data_No_Distinction_err")
...
dico_paras_validation_HDV = from_txt_to_dict("True_Parameters_HDV")
dico_paras_validation_CAV = from_txt_to_dict("True_Parameters_CAV")

change_params_HDV(154,dico_paras_validation_HDV)
change_params_CAV(37968,dico_paras_validation_CAV)

# Remove precedent files
removeFiles("AllAggregated_True_Data_Validation")
removeFiles("AllAggregated_NoDistinction_True_Data_Validation")
